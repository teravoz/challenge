...
function emitEvent(webhook, callback) {
  ...
  .on('response', (response) => {
    if (response.statusCode == 200) {
      console.log(`INFO: POST status code response is ${response.statusCode} for event `, events[i]);
    } else {
      console.warn(`WARN: POST status code response is ${response.statusCode} for event `, events[i]);
    }
    callback(null);
  })
  .on('error', (err) => {
